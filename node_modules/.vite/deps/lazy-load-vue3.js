...
var elLazyKeySetMap = /* @__PURE__ */ new Map();
var lazyKeyVmSetMap = /* @__PURE__ */ new Map();
var lazyKeyElSetMap = /* @__PURE__ */ new Map();
var baseConfig = {
  error: "",
  loading: "",
  errorClassList: [],
...
function handler(sorted, targetSet, checkFn, getEl, updateFn) {
  ...
  try {
    for (_iterator.s(); !(_step = _iterator.n()).done; ) {
      ...
      } else if (viewStatus === 1 && sorted && flag)
        break;
    }
  } catch (err) {
    _iterator.e(err);
...
function addComponentRecords(vm) {
  ...
  var vmSet = lazyKeyVmSetMap.get(lazyKey) || /* @__PURE__ */ new Set();
  if (vmSet.has(vm))
    return;
  vmSet.add(vm);
...
function addDirectiveRecords(el, lazyKey) {
  ...
  var elSet = lazyKeyElSetMap.get(lazyKey) || /* @__PURE__ */ new Set();
  if (elSet.has(el))
    return;
  elSet.add(el);
...
function addListener(parent, lazyKey) {
  while (parent) {
    var lazyKeySet = elLazyKeySetMap.get(parent) || /* @__PURE__ */ new Set();
...
function throttle(cb) {
  ...
  var handler2 = function handler3(sorted, event) {
    if (event && ![window, document].includes(event.target)) {
      ...
      var _iterator2 = _createForOfIteratorHelper(targetVmSets), _step2;
      try {
      ...
      var _iterator3 = _createForOfIteratorHelper(targetElSets), _step3;
      try {
      ...
      var _iterator4 = _createForOfIteratorHelper(lazyKeyVmSetMap), _step4;
      try {
        for (_iterator4.s(); !(_step4 = _iterator4.n()).done; ) {
          var _step4$value = _slicedToArray(_step4.value, 2), vmSet = _step4$value[1];
          cb(
            sorted,
            0,
            vmSet
      ...
      var _iterator5 = _createForOfIteratorHelper(lazyKeyElSetMap), _step5;
      try {
        for (_iterator5.s(); !(_step5 = _iterator5.n()).done; ) {
          var _step5$value = _slicedToArray(_step5.value, 2), elSet = _step5$value[1];
          cb(
            sorted,
            1,
            elSet
...
function findSet(target, map) {
  ...
  var res = [];
  var _iterator6 = _createForOfIteratorHelper(lazyKeySet), _step6;
  try {
    for (_iterator6.s(); !(_step6 = _iterator6.n()).done; ) {
      var lazyKey = _step6.value;
      if (map.has(lazyKey))
        res.push(map.get(lazyKey));
    }
  } catch (err) {
    _iterator6.e(err);
  } finally {
...
var LazyDirective = {
  ...
  updated: function updated(el, _ref2) {
    ...
    if (typeof value === "string") {
      ...
      if (value.src === oldSrc)
        return;
      var loading = value.loading, _value$loadingClassLi2 = value.loadingClassList, loadingClassList = _value$loadingClassLi2 === void 0 ? [] : _value$loadingClassLi2;
      if (loading)
        el.setAttribute("src", loading);
